...
export class SuperAdminController {
  ...
  toggleActive(@Param('id') id: string, @Body() body: { isActive: boolean }) {
    return this.svc.toggleUserActive(id, body.isActive);
  }
  ...
  updateSubscription(@Param('businessId') businessId: string, @Body() body: any) {
    return this.svc.updateSubscription(businessId, body);
  }

  @UseGuards(SuperAdminGuard)
  @Post('subscriptions/:id/payment')
  recordPayment(@Param('id') id: string, @Body() body: { amount: number; method: string; reference?: string }) {
    return this.svc.recordSubscriptionPayment(id, body.amount, body.method, body.reference);
  }
  ...
  @UseGuards(SuperAdminGuard)
  @Get('activity')
  getActivity(@Query('businessId') businessId?: string, @Query('limit') limit?: string) {
    return this.svc.getLoginActivity(businessId, limit ? +limit : 50);
  }
